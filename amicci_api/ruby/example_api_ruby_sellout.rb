...
class Sellout
    ...
    #Required
    attr_accessor :date, :id_product, :id_store, :quantity, :sale_gross, :sale_liquid
        
    #Optional
    attr_accessor :cost_gross, :cost_liquid, :sale_channel

    def initialize date, id_product, id_store, quantity, sale_gross, sale_liquid
        if date.nil?
            raise "Field date is required"
        elsif id_product.nil?
            raise "Field id_product is required"
        elsif id_store.nil?
            raise "Field id_store is required"
        elsif quantity.nil?
            raise "Field quantity is required"
        elsif sale_gross.nil?
            raise "Field sale_gross is required"
        elsif sale_liquid.nil?
            raise "Field sale_liquid is required"
        end
        @date = date
    ...
    for j in (1..MAX_QUANTITY).step(1)
      # Creates an object and adds it to the list
        begin
            ...
            obj = Sellout.new((Date.today-1).strftime('%Y-%m-%d'), (j+i-1), (j+i-1), 15, 30, 20)
