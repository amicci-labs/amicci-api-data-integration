...
class Stock
    ...
    #Required
    attr_accessor :date, :id_product, :id_store, :quantity

    def initialize date, id_product, id_store, quantity
        if date.nil?
    ...
    for j in (1..MAX_QUANTITY).step(1)
      # Creates an object and adds it to the list
        begin
            ...
            obj = Stock.new((Date.today-1).strftime('%Y-%m-%d'), (j+i-1), (j+i-1), 10)
